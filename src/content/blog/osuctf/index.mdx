...
Ok, so there is a vulnerability in `edit_area` where if we supply an out of bounds indices we can do an oob write with data. This is reliant on the size of the struct of the area data, but I'll edit it once i3 tells me more about it again. Additionally providing a negative index for edit_area will print out previous stack values, and we can leverage an index of -3 to get a libc leak.
